...
export default function ProjectPageClient({ project }: { project: Project }) {
  ...
  useEffect(() => {
    ...
    img.onload = () => {
      ...
      else if (ratio >= 0.9) setImageAspectRatio("aspect-square");
            ...
            >
              <div className="w-full space-y-4">
                ...
                {projectTypes.length > 0 && (
                  ...
                  >
                    {projectTypes.map((type, index) => (
                      <span key={index} className="inline-flex items-center rounded-lg bg-white px-3 py-1.5 text-sm font-medium text-neutral-900 border border-neutral-300">
                        {type}
                      </span>
                    ))}
                  </motion.div>
                ...
                {keyFeatures.length > 0 && (
                  ...
                  >
                    <h2 className="text-2xl font-semibold text-neutral-800">Key Features</h2>
                ...
                {project.role && (
                  <motion.div
                    ...
                    className="space-y-2"
                  >
                    <h3 className="text-lg font-semibold text-neutral-800">Role</h3>
                ...
                {technologies.length > 0 && (
                  <motion.div
                    ...
                    className="space-y-3"
                  >
                    <h3 className="text-lg font-semibold text-neutral-800">Technologies</h3>
                    <div className="flex flex-wrap gap-2">
                      {technologies.map((tech, index) => (
                        <span key={index} className="inline-flex items-center rounded-lg bg-neutral-900 px-3 py-1.5 text-sm font-medium text-white relative z-20">
                          {tech}
                        </span>
                      ))}
                    </div>
                ...
                {(project.githubLink || project.caseStudy || project.siteLink) && (
                  ...
                  >
                    <h3 className="text-lg font-semibold text-neutral-800">Links</h3>
            ...
            >
              <div className="relative flex flex-col items-center">
                <div className="w-full max-w-md sm:max-w-lg md:max-w-xl lg:max-w-none">
                  {project.imageUrl ? (
                    isImageLoaded ? (
                      <div className={`relative ${imageAspectRatio} w-full rounded-2xl sm:rounded-3xl transition-opacity duration-500 opacity-100`}>
                        <div className="absolute inset-0 flex items-center justify-center overflow-visible z-0">
                          <FeaturedSlugDesign />
                        </div>
                        <div className={`relative z-10 ${imageAspectRatio} w-full rounded-2xl sm:rounded-3xl select-none ${
